...
def priority_score(row, track=None, company_apply_rate=None):
    ...
    resume_raw = _parse_score(getattr(row, "resume_match_score", None) or "0")
    score += min(resume_raw * 0.4, 40)

    # --- Sponsorship (+15 / +5 / -10) ---
    sponsor = (getattr(row, "sponsorship_status", None) or "").lower()
    if sponsor == "positive":
        score += 15
    ...
    age = _parse_age(getattr(row, "posted_age_days", None))
    if age <= 7:
        score += 15
    elif age <= 14:
        score += 10
    ...
    jd_len_attr = getattr(row, "jd_text_length", None)
    ...
    ats = (getattr(row, "ats_group", None) or "").lower()
    score += ATS_SUCCESS_ADJUSTMENTS.get(ats, 0)

    # --- Search bucket adjustment ---
    bucket = (getattr(row, "search_bucket", None) or "").lower().replace(" ", "_")
...
def improved_triage(row, track=None, score=0):
    ...
    if getattr(row, "is_duplicate", False):
        return "skip"
    if (getattr(row, "sponsorship_status", None) or "").lower() == "negative":
        return "skip"
