...
def export_document_bundle(
    ...
    """Export v2 portable metadata together with immutable document bytes."""
    operation_id = str(uuid4())
    started = perf_counter()
    try:
        content = export_backup_bundle(db, user.id)
        ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        logger.info(
            "backup_bundle_export operation_id=%s outcome=success bytes=%s elapsed_ms=%s",
            operation_id,
            len(content),
            int((perf_counter() - started) * 1000),
        )
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="jobgrid_backup_bundle_{ts}.zip"'
                )
            },
        )
    except BackupContractError as exc:
        logger.warning(
            "backup_bundle_export operation_id=%s outcome=%s elapsed_ms=%s",
            operation_id,
            exc.code,
            int((perf_counter() - started) * 1000),
        )
...
@router.post("/backup/import/bundle")
async def import_document_bundle(
    ...
    """Validate and restore a v2 document bundle."""
    operation_id = str(uuid4())
    started = perf_counter()
    try:
        raw = await file.read(MAX_BACKUP_BUNDLE_UPLOAD_BYTES + 1)
        ...
        result = restore_backup_bundle(db, user.id, raw, mode)
        affected = sum(values["created"] for values in result["counts"].values())
        logger.info(
            "backup_bundle_restore operation_id=%s outcome=success mode=%s affected=%s elapsed_ms=%s",
            operation_id,
            mode,
            affected,
            int((perf_counter() - started) * 1000),
        ...
        logger.warning(
            "backup_bundle_restore operation_id=%s outcome=%s mode=%s elapsed_ms=%s",
            operation_id,
            exc.code,
            mode,
            int((perf_counter() - started) * 1000),
        ...
        logger.exception(
            "backup_bundle_restore operation_id=%s outcome=restore_failed mode=%s elapsed_ms=%s",
            operation_id,
            mode,
            int((perf_counter() - started) * 1000),
        )
        raise HTTPException(
            ...
            detail={
                "code": "restore_failed",
                "message": "Document bundle restore failed safely.",
            },
        ) from exc
    finally:
        await file.close()
