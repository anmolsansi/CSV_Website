...
from ..models import AuditEvent, JobTrack, ReminderDelivery
...
def prune_mutation_receipts(
    ...
    *,
    ...
    return (
        db.query(MutationReceipt)
        .filter(MutationReceipt.created_at < cutoff)
    ...
    if q:
        ...
        query = query.filter(
            or_(
                ...
                Contact.company_display.ilike(pattern),
            )
        )
    return [
        serialize_contact(item)
        for item in query.order_by(Contact.name.asc(), Contact.id.asc()).limit(limit).all()
    ]
    ...
    *,
    ...
    if contact.version != version:
        raise ContactServiceError(
            "stale_version",
            409,
            "Contact changed. Reload and retry.",
        )
...
def list_application_contacts(
    ...
    *,
    ...
    rows = (
        db.query(ApplicationContact, Contact)
    ...
    *,
    ...
    if replay:
        row = (
            ...
            .filter(
                ApplicationContact.id == int(replay.result_entity_id),
                ApplicationContact.user_id == user_id,
            )
            .first()
        )
...
def unlink_application_contact(
    db: Session,
    *,
    user_id: int,
    track_id: int,
    ...
    if row is None:
        raise ContactServiceError(
            "not_found",
            404,
            "Application contact link not found.",
        )
    db.delete(row)
    db.flush()
...
def serialize_interview(db: Session, interview: Interview) -> dict[str, Any]:
    ...
    if interview.contact_id is not None:
        contact = (
            db.query(Contact)
            .filter(
                Contact.id == interview.contact_id,
                Contact.user_id == interview.user_id,
            )
            .first()
        )
    overlaps = (
    ...
    return {
        ...
        "overlap_interview_ids": overlaps,
        "warnings": (
            [{"code": "overlap", "interview_ids": overlaps}]
            if overlaps
            else []
        ),
...
def list_interviews(
    ...
    *,
    ...
    rows = (
        db.query(Interview)
        .filter(Interview.user_id == user_id, Interview.track_id == track_id)
...
def _aware_for_validation(value: datetime) -> datetime:
    if value.tzinfo is None or value.utcoffset() is None:
        return value.replace(tzinfo=timezone.utc)
    return value.astimezone(timezone.utc)


def _cancel_unsent_interview_reminders(
    db: Session,
    *,
    user_id: int,
    interview_id: int,
    ...
    rows = (
        ...
        .filter(
            ...
            ReminderDelivery.occurrence_key.like(f"interview:{interview_id}:%"),
    ...
    for row in rows:
        ...
        row.last_error_code = "interview_changed"
        row.version += 1
    if rows:
        db.flush()
    return len(rows)
    ...
    db.add(
        AuditEvent(
            ...
            metadata_json={
                ...
                "status": interview.status,
                "version": interview.version,
                "cancelled_unsent_reminders": cancelled_reminders,
            },
        )
    )
    ...
    *,
    ...
    if interview.version != version:
        raise ContactServiceError(
            "stale_version",
            409,
            "Interview changed. Reload and retry.",
        )
    if "contact_id" in changes and changes["contact_id"] is not None:
        _owned_contact(db, user_id, int(changes["contact_id"]))

    start_value = changes.get("starts_at", interview.starts_at)
    end_value = changes.get("ends_at", interview.ends_at)
    if start_value is None or end_value is None:
        raise ContactServiceError(
            "invalid_interview_range",
            422,
            "Interview start and end cannot be null.",
        )
    starts_at, ends_at = validate_interview_window(
        _aware_for_validation(start_value),
        _aware_for_validation(end_value),
    )

    normalized = {**changes, "starts_at": starts_at, "ends_at": ends_at}
    for key, value in normalized.items():
        setattr(interview, key, value)
    interview.version += 1
    interview.updated_at = datetime.utcnow()

    cancelled_reminders = _cancel_unsent_interview_reminders(
        db,
        user_id=user_id,
